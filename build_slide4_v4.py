"""Slide 4 v16: TCO panel rebuilt as NATIVE PowerPoint elements
(no embedded image). Bars, labels, arrows, KPI tile are all individual
editable shapes. Click animations preserved.
"""
...
DST = "/tmp/2026_05_18_VS_Zielbild_SNOW_SPM_v16.pptx"
...
GRID_GREY = RGBColor(0xE5, 0xE7, 0xEB)
...
def add_arrow(x1, y1, x2, y2, color, weight=2.0, both=False):
    """Insert a straight connector arrow."""
...
GARTNER_GROUP = []
DIM_GROUP = []
def track(shape, group): group.append(shape.shape_id); return shape
...
for i, (krit, snow, bob) in enumerate(rows):
    ...
    add_text(COL_X + 1.2, y, COL_W[0] - 2, row_h, [
        (krit, dict(size=10.5, bold=True, color=WHITE))], anchor=MSO_ANCHOR.MIDDLE)
    add_rect(colx(1), y, 0.6, row_h, ORANGE)
    if "integ" in snow.lower():
        ...
        add_text(colx(1) + 1.2, y, COL_W[1] - 2, row_h, [
            (snow, dict(size=10.5, color=DARK))], anchor=MSO_ANCHOR.MIDDLE)
    add_text(colx(2) + 1.2, y, COL_W[2] - 2, row_h, [
        (bob, dict(size=10.5, color=GREY_TXT))], anchor=MSO_ANCHOR.MIDDLE)
...
# ===== NATIVE CHART AREA =====
# Layout within TCO body:
#   left part: bar chart (5-Year TCO)
#   right part: KPI tile (-26 % p.a.)
INNER_PAD = 1.8
INNER_X = tco_x + INNER_PAD
INNER_Y = chart_top + INNER_PAD
INNER_W = left_w - 2 * INNER_PAD
INNER_H = chart_body_h - 2 * INNER_PAD

# Split: 64% chart, 36% KPI
chart_w = INNER_W * 0.62
gap_inner = 1.0
kpi_w = INNER_W - chart_w - gap_inner

# ---------- LEFT: 5-Year TCO bar chart ----------
chart_x = INNER_X
title_h = 2.4
track(add_text(chart_x, INNER_Y, chart_w, title_h, [
    ("5-Jahres-TCO  (Year 0–5)",
     dict(size=11, bold=True, color=DARK))], anchor=MSO_ANCHOR.MIDDLE), TCO_GROUP)

# Chart axis area
caption_h = 2.0
xlbl_h = 3.0           # labels below bars (two lines)
val_lbl_h = 2.0        # value above bar
bars_y_top = INNER_Y + title_h + val_lbl_h + 0.3
bars_y_bot = INNER_Y + INNER_H - xlbl_h - caption_h - 0.4
bars_h_max = bars_y_bot - bars_y_top  # full vertical space for value scale

# Baseline
track(add_rect(chart_x + 4.0, bars_y_bot - 0.05, chart_w - 4.5, 0.1, GREY_TXT),
      TCO_GROUP)

# Y-axis: simple tick marks at 0, 2, 4, 6, 8 M€
MAX_Y = 8.0
def y_for(value): return bars_y_bot - (value / MAX_Y) * bars_h_max
for v in [0, 2, 4, 6, 8]:
    y = y_for(v)
    # tick line (subtle horizontal gridline)
    track(add_rect(chart_x + 4.0, y - 0.04, chart_w - 4.5, 0.08, GRID_GREY),
          TCO_GROUP)
    # axis label
    track(add_text(chart_x, y - 1.2, 4.0, 2.4, [
        (f"{v}", dict(size=8.5, color=GREY_TXT, align=PP_ALIGN.RIGHT))],
        anchor=MSO_ANCHOR.MIDDLE), TCO_GROUP)
# Y-axis caption (top-left)
track(add_text(chart_x, INNER_Y + title_h + 0.2, 6, 1.8, [
    ("Mio. €", dict(size=8.5, italic=True, color=GREY_TXT))],
    anchor=MSO_ANCHOR.MIDDLE), TCO_GROUP)

# Bars
bars_area_x = chart_x + 5.5
bars_area_w = chart_w - 6.0
bar_w = bars_area_w / 4.0   # 4 slots: left margin + bar + gap + bar (then margin)
SN_V  = 4.78
BOB_V = 7.02
DELTA = BOB_V - SN_V       # 2.24
PCT_5J = DELTA / BOB_V * 100  # ~32

# Position bars: bar 1 at slot1, bar 2 at slot3
bar1_x = bars_area_x + bar_w * 0.4
bar2_x = bars_area_x + bar_w * 2.4

bar1_h = (SN_V / MAX_Y) * bars_h_max
bar2_h = (BOB_V / MAX_Y) * bars_h_max
bar1_y = bars_y_bot - bar1_h
bar2_y = bars_y_bot - bar2_h
bar_actual_w = bar_w * 1.2

# SN bar (orange)
track(add_rect(bar1_x, bar1_y, bar_actual_w, bar1_h, ORANGE,
               shape=MSO_SHAPE.ROUNDED_RECTANGLE), TCO_GROUP)
# Value label above SN bar
track(add_text(bar1_x - 1, bar1_y - 2.2, bar_actual_w + 2, 2.0, [
    (f"{SN_V:.2f} M€", dict(size=11, bold=True, color=DARK, align=PP_ALIGN.CENTER))],
    anchor=MSO_ANCHOR.BOTTOM), TCO_GROUP)
# X-axis label SN
track(add_text(bar1_x - 2, bars_y_bot + 0.4, bar_actual_w + 4, xlbl_h, [
    ("ServiceNow",
     dict(size=10, color=DARK, align=PP_ALIGN.CENTER)),
    ("(SPM-Plattform)",
     dict(size=8.5, italic=True, color=GREY_TXT, align=PP_ALIGN.CENTER))],
    anchor=MSO_ANCHOR.TOP), TCO_GROUP)

# BoB bar (navy)
track(add_rect(bar2_x, bar2_y, bar_actual_w, bar2_h, NAVY,
               shape=MSO_SHAPE.ROUNDED_RECTANGLE), TCO_GROUP)
track(add_text(bar2_x - 1, bar2_y - 2.2, bar_actual_w + 2, 2.0, [
    (f"{BOB_V:.2f} M€", dict(size=11, bold=True, color=DARK, align=PP_ALIGN.CENTER))],
    anchor=MSO_ANCHOR.BOTTOM), TCO_GROUP)
track(add_text(bar2_x - 2, bars_y_bot + 0.4, bar_actual_w + 4, xlbl_h, [
    ("Best-of-Breed",
     dict(size=10, color=DARK, align=PP_ALIGN.CENTER)),
    ("Einzeltools",
     dict(size=8.5, italic=True, color=GREY_TXT, align=PP_ALIGN.CENTER))],
...
# Delta annotation: arrow between bars + text above
mid_y = (bar1_y + bars_y_bot) / 2  # roughly middle of SN bar height
arrow = add_arrow(bar1_x + bar_actual_w + 0.2, mid_y,
                  bar2_x - 0.2, mid_y, GREEN, weight=2.0, both=True)
TCO_GROUP.append(arrow.shape_id)
delta_text_x = bar1_x + bar_actual_w
delta_text_w = bar2_x - delta_text_x
track(add_text(delta_text_x - 6, mid_y - 5.5, delta_text_w + 12, 3.8, [
    (f"Ersparnis  −{DELTA:.2f} M€",
     dict(size=10.5, bold=True, color=GREEN, align=PP_ALIGN.CENTER)),
    (f"( −{PCT_5J:.0f} % )",
     dict(size=10, bold=True, color=GREEN, align=PP_ALIGN.CENTER))],
    anchor=MSO_ANCHOR.BOTTOM), TCO_GROUP)

# ---------- RIGHT: KPI tile ----------
kpi_x = INNER_X + chart_w + gap_inner
# Title
track(add_text(kpi_x, INNER_Y, kpi_w, title_h, [
    ("Laufend p.a.",
...
PCT_Y3 = 26
DELTA_Y3 = 345  # k€
kpi_box_top = INNER_Y + title_h + 0.5
kpi_box_h = INNER_H - title_h - caption_h - 1.0
track(add_rect(kpi_x + 0.4, kpi_box_top, kpi_w - 0.8, kpi_box_h, WHITE,
               shape=MSO_SHAPE.ROUNDED_RECTANGLE, line=GREEN, line_w=2.0),
      TCO_GROUP)

# Big percent number
track(add_text(kpi_x + 0.4, kpi_box_top + kpi_box_h * 0.10,
               kpi_w - 0.8, kpi_box_h * 0.50, [
    (f"−{PCT_Y3} %",
     dict(size=36, bold=True, color=GREEN, align=PP_ALIGN.CENTER))],
    anchor=MSO_ANCHOR.MIDDLE), TCO_GROUP)
# Sub-label
track(add_text(kpi_x + 0.4, kpi_box_top + kpi_box_h * 0.62,
               kpi_w - 0.8, 3.0, [
    (f"= −{DELTA_Y3} k€ p.a.",
     dict(size=12, bold=True, color=DARK, align=PP_ALIGN.CENTER))],
    anchor=MSO_ANCHOR.MIDDLE), TCO_GROUP)
track(add_text(kpi_x + 0.4, kpi_box_top + kpi_box_h * 0.78,
               kpi_w - 0.8, 2.5, [
    ("weniger Betriebskosten",
     dict(size=9, italic=True, color=GREY_TXT, align=PP_ALIGN.CENTER))],
    anchor=MSO_ANCHOR.MIDDLE), TCO_GROUP)

# Caption under whole chart area
track(add_text(INNER_X, INNER_Y + INNER_H - caption_h + 0.2, INNER_W, caption_h, [
    ("Quelle: TCO_Vergleich_5J_SN_vs_BoB.xlsx — Tab 'Auswertung'   |   Werte basieren auf Annahmen / Schätzungen",
     dict(size=7.5, italic=True, color=GREY_TXT, align=PP_ALIGN.CENTER))],
    anchor=MSO_ANCHOR.MIDDLE), TCO_GROUP)
...
for i, (head, body) in enumerate(statements):
    ...
    track(add_rect(gar_x + 1.5, y, right_w - 3, sb_h - 0.8, WHITE,
                   shape=MSO_SHAPE.ROUNDED_RECTANGLE, line=ORANGE, line_w=1.2),
          GARTNER_GROUP)
...
def make_click_group(click_inner_tn_id, spids):
    ...
    inner = par.find('.//' + qn('p:par') + '/' + qn('p:cTn') + '/' + qn('p:childTnLst'))
...
bld_xml_parts = [f'<p:bldLst xmlns:p="{P_NS}">']
...
prs.save(DST)
print("DIM:", len(DIM_GROUP), "TCO shapes:", len(TCO_GROUP),
      "GARTNER shapes:", len(GARTNER_GROUP))
print("Saved:", DST)
