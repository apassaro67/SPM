...
DST = "/tmp/2026_07_08_Business_Case_SEPM_v5.pptx"
...
def s_a2_uebersicht():
    s = add_stihl("AUFGABE 2 — 20 Schluesselprozesse @ STIHL: Uebersicht + Harmonisierungspotenzial")
    R(s, CL, CT, CW, 0.50, DARK, shape=MSO_SHAPE.ROUNDED_RECTANGLE)
    T(s, CL + 0.15, CT, 4.5, 0.50, [("Bewertungsergebnis Excel-Analyse", dict(size=11.5, bold=True, color=WHITE))], anchor=MSO_ANCHOR.MIDDLE)
    lx = CL + 4.70
    legend = [("Sehr Hoch", GREEN), ("Hoch", GREEN_MID), ("Mittel-Hoch", YELLOW_GRN), ("Mittel", ORANGE), ("Gering", RED_HL), ("na", GREY_NA)]
    for lbl, col in legend:
        R(s, lx, CT + 0.14, 0.24, 0.22, col, shape=MSO_SHAPE.ROUNDED_RECTANGLE)
        T(s, lx + 0.30, CT, 1.15, 0.50, [(lbl, dict(size=9, bold=True, color=WHITE))], anchor=MSO_ANCHOR.MIDDLE)
        lx += 1.35
    ...
    for gi, gname in enumerate(gruppen):
        ...
        R(s, x, top, w, 0.42, color, shape=MSO_SHAPE.ROUNDED_RECTANGLE)
        items = [p for p in PROC if p[1] == gname]
        T(s, x + 0.15, top + 0.02, w - 0.30, 0.38, [(f"{gname.upper()}  ({len(items)} Prozesse)", dict(size=12, bold=True, color=WHITE))], anchor=MSO_ANCHOR.MIDDLE)
        row_h = min((h - 0.60) / max(len(items), 1), 0.70)
        for j, p in enumerate(items):
            ...
            O(s, x + 0.20, y + (row_h - 0.05) / 2 - 0.13, 0.26, 0.26, color)
            T(s, x + 0.20, y + (row_h - 0.05) / 2 - 0.13, 0.26, 0.26, [(str(nr), dict(size=8.5, bold=True, color=WHITE, align=PP_ALIGN.CENTER))], anchor=MSO_ANCHOR.MIDDLE)
            T(s, x + 0.55, y + 0.02, w - 2.50, row_h - 0.10, [(prz, dict(size=9.5, bold=True, color=DARK))], anchor=MSO_ANCHOR.MIDDLE)
            R(s, x + w - 1.85, y + (row_h - 0.05) / 2 - 0.13, 0.55, 0.26, GREY_BG, shape=MSO_SHAPE.ROUNDED_RECTANGLE)
            T(s, x + w - 1.85, y + (row_h - 0.05) / 2 - 0.13, 0.55, 0.26, [(anz, dict(size=8, bold=True, color=DARK, align=PP_ALIGN.CENTER))], anchor=MSO_ANCHOR.MIDDLE)
            pcol = POT_COLOR[pot]
            R(s, x + w - 1.20, y + (row_h - 0.05) / 2 - 0.13, 1.05, 0.26, pcol, shape=MSO_SHAPE.ROUNDED_RECTANGLE)
            T(s, x + w - 1.20, y + (row_h - 0.05) / 2 - 0.13, 1.05, 0.26, [(pot, dict(size=7.5, bold=True, color=WHITE, align=PP_ALIGN.CENTER))], anchor=MSO_ANCHOR.MIDDLE)
    counts = {}
    for p in PROC: counts[p[7]] = counts.get(p[7], 0) + 1
    T(s, CL, CB - 0.05, CW, 0.20, [(f"Zusammenfassung  ·  Sehr Hoch: {counts.get('Sehr Hoch',0)}  ·  Hoch: {counts.get('Hoch',0)}  ·  Mittel-Hoch: {counts.get('Mittel-Hoch',0)}  ·  Mittel: {counts.get('Mittel',0)}  ·  na: {counts.get('na',0)}", dict(size=9, italic=True, color=GREY_TXT, align=PP_ALIGN.CENTER))], anchor=MSO_ANCHOR.MIDDLE)
