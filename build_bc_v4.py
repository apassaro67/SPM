...
DST = "/tmp/2026_07_08_Business_Case_SEPM_v4.pptx"
...
def s_toc():
    ...
    chapters = [
        ...
        ("A2", "AUFGABE 2 — SNOW-Prozessabdeckung",
         [(5, "Schluesselprozesse @ STIHL",       NAVY),
          (6, "Reifebewertung ServiceNow",        NAVY),
          (7, "Alternativen (mit Nachteilen)",    NAVY)]),
        ("A3", "AUFGABE 3 — Governance-Strukturen",
         [(8,  "Governance-Modelle A / B / C",             NAVY),
          (9,  "Was aendert sich pro Schluesselprozess",   NAVY),
          (10, "Benefits (Zeit · Qualitaet · Kosten)",     NAVY),
          (11, "Einmalaufwaende + Betriebsthemen",         NAVY)]),
        ("A4", "AUFGABE 4 — Plattform-Abhaengigkeit",
         [(12, "Risikomatrix neu (BoB vs. Plattform)",     NAVY),
          (13, "Lockin Pro/Con + Anpassungsaufwaende",     NAVY),
          (14, "SAP-SNOW Vergleich + Vertragshistorie",    NAVY),
          (15, "Referenzkunden",                            NAVY)]),
        ("G",  "GESAMTDARSTELLUNG",
         [(16, "Qualitative Analyse (5 Dimensionen)",      ORANGE),
          (17, "Strategische Ausrichtung / neue Geschaeftsfelder", ORANGE),
          (18, "TCO und Betrieb",                          ORANGE),
          (19, "Umsetzungsvorschlag Szenario 1 + 2",       ORANGE),
          (20, "Empfehlung + Entscheidungsvorlage",        ORANGE)]),
    ]
    top = CT
    lw = (CW - 0.30) / 2
    rx = CL + lw + 0.30
...
# ================================
# 5 · AUFGABE 2 — SCHLUESSELPROZESSE
# ================================
def s_a2_prozesse():
    s = add_stihl("AUFGABE 2 — Schluesselprozesse @ STIHL  (aus Excel-Liste)")
    R(s, CL, CT, CW, 0.55, GREEN_LT, shape=MSO_SHAPE.ROUNDED_RECTANGLE)
    R(s, CL, CT, 0.08, 0.55, GREEN)
    T(s, CL + 0.15, CT, CW - 0.25, 0.55, [
        ("Basis fuer die Reife-Bewertung: 8 zentrale STIHL-Schluesselprozesse rund um das Portfolio-Management",
         dict(size=10.5, italic=True, color=DARK))], anchor=MSO_ANCHOR.MIDDLE)
    prozesse = [
        ("Strategisches Portfolio-\nManagement",  "OKR-Verfolgung, Konzern-Priorisierung",         ORANGE),
        ("Demand-Management",                     "Idee bis Vorhaben, Freigabe-Workflow",           ORANGE),
        ("Projekt- & Ressourcen-\nplanung",       "Ressourcen, Skills, Kapazitaeten, Gantt/Agile",  ORANGE),
        ("Reporting & Dashboards",                "Ampel-Reporting, KPI-Dashboards Vorstand",       ORANGE),
        ("Entwicklungsprojekte (PEP)",            "Stage-Gate, Reifegrade, Product-Backlog",        NAVY),
        ("EV-Prozess",                            "Entscheidungsvorlage, Freigabe-Workflow",        NAVY),
        ("Application Portfolio Mgmt",            "Anwendungs-Lifecycle, Kosten, TCO je App",       NAVY),
        ("Massnahmen-Management",                 "Zielkosten-Steuerung Produktentw. (Hype!)",      RED_HL),
    ]
    top = CT + 0.75
    w = (CW - 0.45) / 4
    h = 2.60
    gap = 0.15
    for i, (name, desc, color) in enumerate(prozesse):
        row = i // 4; col = i % 4
        x = CL + col * (w + gap)
        y = top + row * (h + gap)
        R(s, x, y, w, h, WHITE, shape=MSO_SHAPE.ROUNDED_RECTANGLE, line=color, lw=1.2)
        R(s, x, y, w, 0.06, color)
        # number circle
        O(s, x + 0.12, y + 0.20, 0.40, 0.40, color)
        T(s, x + 0.12, y + 0.20, 0.40, 0.40, [
            (str(i + 1), dict(size=13, bold=True, color=WHITE, align=PP_ALIGN.CENTER))],
            anchor=MSO_ANCHOR.MIDDLE)
        T(s, x + 0.60, y + 0.15, w - 0.75, 1.20, [
            (name, dict(size=11.5, bold=True, color=DARK))],
            anchor=MSO_ANCHOR.MIDDLE)
        T(s, x + 0.15, y + 1.50, w - 0.30, h - 1.65, [
            (desc, dict(size=9.5, italic=True, color=GREY_TXT))],
            anchor=MSO_ANCHOR.TOP)
    # Legend
    T(s, CL, 6.75, CW, 0.24, [
        ("Farbe:  orange = klassische PPM-Prozesse  ·  navy = SNOW-nahe Erweiterung  ·  rot = out of scope (Hype)",
         dict(size=9, italic=True, color=GREY_TXT))])
s_a2_prozesse()

# ================================
# 6 · AUFGABE 2 — REIFEBEWERTUNG
# ================================
def s_a2_reife():
    s = add_stihl("AUFGABE 2 — Reifebewertung ServiceNow pro Schluesselprozess")
    hdr_y = CT
    R(s, CL, hdr_y, CW, 0.32, DARK, shape=MSO_SHAPE.ROUNDED_RECTANGLE)
    hcols = [("Schluesselprozess", 4.0), ("SNOW-Modul / Ansatz", 3.5),
             ("Reifegrad (1-5)", 2.5), ("Kommentar / Handlungsbedarf", 2.67)]
    xc = CL
    for name, w in hcols:
        ...
        xc += w
    reifen = [
        ("Strategisches Portfolio-Mgmt",   "SPM Pro · Strategic Planning",     5, "Standard, direkt einsetzbar"),
        ("Demand-Management",              "SPM Pro · Demand Mgmt",            5, "Standard, direkt einsetzbar"),
        ("Projekt- & Ressourcenplanung",   "SPM Pro · Project & Resource",     4, "Standard, kleinere Anpassung fuer VEW/VPM"),
        ("Reporting & Dashboards",         "SPM Pro · Performance Analytics",  5, "Standard, Live-Dashboards"),
        ("Entwicklungsprojekte (PEP)",     "SPM Pro + App Engine",             3, "Stage-Gate teils Standard, PEP-Detail via App Engine"),
        ("EV-Prozess",                     "App Engine (Low-Code Workflow)",   3, "kein Modul, aber gut mit App Engine baubar"),
        ("Application Portfolio Mgmt",     "SNOW APM (bereits vorhanden)",     4, "Erweiterung des vorhandenen ITSM/APM"),
        ("Massnahmen-Management",          "— (out of scope)",                 0, "Hype bleibt, siehe Aufgabe 1"),
    ]
    for i, (prozess, modul, reife, komment) in enumerate(reifen):
        y = hdr_y + 0.36 + i * 0.66
        bg = LIGHT_BG if i % 2 == 0 else ROW_ALT
        R(s, CL, y, CW, 0.62, bg, shape=MSO_SHAPE.ROUNDED_RECTANGLE)
        # Prozess
        T(s, CL + 0.15, y, 3.85, 0.62, [
            (prozess, dict(size=10.5, bold=True, color=DARK))],
            anchor=MSO_ANCHOR.MIDDLE)
        # Modul
        T(s, CL + 4.10, y, 3.35, 0.62, [
            (modul, dict(size=9.5, italic=True, color=GREY_TXT))],
            anchor=MSO_ANCHOR.MIDDLE)
        # Stars
        star_start = CL + 7.75
        for k in range(5):
            filled = k < reife
            color = GREEN if reife >= 4 else (ORANGE if reife >= 3 else (YELLOW if reife >= 1 else LIGHT_GREY))
            O(s, star_start + k * 0.36, y + 0.16, 0.30, 0.30,
              color if filled else LIGHT_GREY)
        # Reife-Zahl
        T(s, star_start + 5 * 0.36 + 0.10, y, 0.6, 0.62, [
            (f"{reife}/5" if reife > 0 else "—",
             dict(size=10, bold=True, color=DARK, align=PP_ALIGN.CENTER))],
            anchor=MSO_ANCHOR.MIDDLE)
        # Kommentar
        T(s, CL + 10.35, y, 2.60, 0.62, [
            (komment, dict(size=9, color=DARK))],
            anchor=MSO_ANCHOR.MIDDLE)
    # Fazit
    fy = hdr_y + 0.36 + len(reifen) * 0.66 + 0.15
    R(s, CL, fy, CW, 0.55, GREEN_LT, shape=MSO_SHAPE.ROUNDED_RECTANGLE)
    R(s, CL, fy, 0.08, 0.55, GREEN)
    T(s, CL + 0.15, fy, CW - 0.25, 0.55, [
        ("Fazit:  6 von 7 Prozessen mit Reifegrad ≥ 3 durch SNOW SPM Pro + App Engine abdeckbar. "
         "Kein Kernprozess erfordert Standardisierung 'um jeden Preis'.",
         dict(size=10, italic=True, color=DARK))], anchor=MSO_ANCHOR.MIDDLE)
s_a2_reife()

# ================================
# 7 · AUFGABE 2 — ALTERNATIVEN
# ================================
